      ...
      <div class="p-5">
        ...
        <div class="mt-2 mb-3">
          <span class="text-xs text-gray-400"
            >Created on {{ formatDate(form.createdAt) }}</span
          >
          <span class="ml-3 text-xs font-medium text-secondary-600"
        ...
        <template v-if="!compact || (compact && form.questions.length <= 2)">
          <ul class="mt-3 space-y-1">
            <li
              v-for="(question, index) in form.questions.slice(
                0,
                compact ? 1 : 3
              )"
              :key="question.id"
            ...
            <li
              v-if="form.questions.length > (compact ? 1 : 3)"
              class="text-xs text-gray-500"
            >
              ...and {{ form.questions.length - (compact ? 1 : 3) }} more
              questions
            </li>
          </ul>
        </template>
        <template v-else>
          <p class="text-sm text-gray-400">
            {{ form.questions.length }} questions available
          </p>
        </template>

        <div class="mt-4 flex flex-wrap gap-2">
          <app-share-button
            :url="`${location}/forms/${form.id}`"
            variant="secondary"
            title="Share"
          />
          <app-button variant="outline" type="button">
            <router-link :to="`/forms/${form.id}/results`">
              View Responses
            </router-link>
          </app-button>
          <app-button variant="text" type="button">
            <router-link :to="`/forms/${form.id}`"> Preview </router-link>
          </app-button>
        </div>
      </div>
    </div>
