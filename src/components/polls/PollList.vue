...
<template>
  <div class="space-y-4">
    <div v-if="polls.length === 0" class="text-center py-6">
      <p class="text-gray-500 dark:text-gray-400">{{ compact ? 'No polls created yet' : 'No polls created yet. Create your first poll above!' }}</p>
    </div>
    
    <div v-else v-for="poll in polls" :key="poll.id" 
      :class="[
    ...
    >
      <div class="p-5">
        <div class="flex justify-between items-start">
          <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ poll.question }}</h3>
          <button
        ...
        <div class="mt-2 mb-3">
          <span class="text-xs text-gray-500 dark:text-gray-400">Created on {{ formatDate(poll.createdAt) }}</span>
          <span class="ml-3 text-xs font-medium text-primary-600 dark:text-primary-400">{{ poll.totalVotes }} votes</span>
        </div>
          ...
          <ul class="space-y-2">
            <li v-for="option in poll.options" :key="option.id" class="text-sm">
              <div class="flex justify-between">
                ...
                <span class="text-gray-500 dark:text-gray-400">{{ option.count }} votes</span>
              </div>
        ...
        <template v-else>
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ poll.options.length }} options available</p>
        </template>
